...
export class ChangeScheduleComponent implements OnInit {
  ...
  addHours_workday(){
    ...
    if(this.time != null){
      if(this.time.hour.toString().length == 1){
        if(this.time.minute.toString().length == 1){
          var converted = "0" + this.time.hour.toString() + ":" + "0" + this.time.minute.toString();
        }else{
          var converted = "0" + this.time.hour.toString() + ":" + this.time.minute.toString();
        }
    ...
    addHours_saturday(){
      ...
      var collator = new Intl.Collator(undefined, {numeric: true, sensitivity: 'base'});
      if(this.time != null){
        if(this.time2.hour.toString().length == 1){
          if(this.time2.minute.toString().length == 1){
            var converted = "0" + this.time2.hour.toString() + ":" + "0" + this.time2.minute.toString();
          }else{
            var converted = "0" + this.time2.hour.toString() + ":" + this.time2.minute.toString();
          }
    ...
    addHours_sunday(){
      var collator = new Intl.Collator(undefined, {numeric: true, sensitivity: 'base'});
      if(this.time != null){
        if(this.time3.hour.toString().length == 1){
          if(this.time3.minute.toString().length == 1){
            var converted = "0" + this.time3.hour.toString() + ":" + "0" + this.time3.minute.toString();
          }else{
            var converted = "0" + this.time3.hour.toString() + ":" + this.time3.minute.toString();
          }
